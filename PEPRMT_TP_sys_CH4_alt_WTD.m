...
%Delta
%Written by Patty Oikawa, modified by Sam Chamberlain (2017)
...
%WT_2_adj=WT_2_adj/mean(WT_2_adj);    %changed to normalized water table height by SDC
...
%Labile C pool
M_alpha2 = param_set(2,1);% umol m-3 s-1
M_ea2 = param_set(2,2);
M_km2 = param_set(2,3);
...
GPPmax=max(GPP_2);%parameter for plant-mediated transport and priming (SDC addition)
...
priming_coef = priming_coef - min(priming_coef) + 1; %set baseline to 1 (no priming)
...
%Vtrans=zeros(1,length(Time_2)); %Did not work in model (SDC edit)
%Oxi_factor=zeros(1,length(Time_2)); %Did not work in model (SDC edit)
...
%Alteration for Fe presence
    ...
    M2(t) = M2(t)*M_percent_reduction_3(t); %umol m2 sec (affects plant root zone methanogenesis only; SDC addition)
   
    %incorporate a priming effect; SDC addition
    M1(t) = M1(t)*priming_coef(t); %priming effect
    M2(t) = M2(t)*priming_coef(t); %priming effect
...
for t = 1:length(Time_2)
    ...
    if t==1
        ...
        if WT_2_adj(t)>1  %if you have water, then the CH4 should mix between the 2 layers and concentrations should be the same in water and soil
            ...
            CH4water_0_2(t)= M_full(t)+CH4water_0(t);%umol per m^3 - concentration in water doesn't change
