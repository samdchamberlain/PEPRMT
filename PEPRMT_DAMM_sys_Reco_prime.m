...
%choose measured or modelled GPP. Use measured for now to reduce sources of
%error
GPP_2 = abs(GPP);
GPPmax=max(GPP_2); %parameter for priming calculation

%priming coefficient
priming_coef = (1.5*(GPP_2/GPPmax)); %set winter values to zero
priming_coef = priming_coef - min(priming_coef) + 1;
...
NPPsum_avail_2 = (GPP_2*60*30)*0.5;%umol m-2 30min-1 --give Reco access to half GPP

%other half of GPP needs to be lost as autotrophic respiration
Auto_resp = (GPP_2*60*30)*0.15; %umol m-2 30min-1
...
%initialize C pools
C1_init = SOC; %total C  in umol m-3, initially fixed at 3e9 (SDC edit)
C2_init = 0;
...
if percent_reduction(t)<0.75
   percent_reduction(t)=0.75;
    ...
    R1(t) = R1(t)*priming_coef(t); %calc priming effect
    R2(t) = R2(t)*priming_coef(t); %calc priming effect
